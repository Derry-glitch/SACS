class EventModel {
  ...
  factory EventModel.fromJson(Map<String, dynamic> json) {
    // Helper to map C# enum index or string to display name
    String parseEventType(dynamic val) {
      if (val is int) {
        const types = ['Assignment', 'Quiz', 'Exam', 'Project', 'StudySession'];
        if (val >= 0 && val < types.length) {
          return types[val];
        }
        return 'Assignment';
      }
      return val?.toString() ?? 'Assignment';
    }
    ...
    return EventModel(
      ...
      priorityLevel: json['priorityLevel']?.toString() ?? 'Medium',
