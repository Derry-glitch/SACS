...
public record GetDailyCalendarQuery(DateTime? Date) : IRequest<IEnumerable<CalendarEventDto>>;

public class GetDailyCalendarQueryHandler : IRequestHandler<GetDailyCalendarQuery, IEnumerable<CalendarEventDto>>
{
    private readonly IUnitOfWork _unitOfWork;
    private readonly ICurrentUserService _currentUserService;

    public GetDailyCalendarQueryHandler(IUnitOfWork unitOfWork, ICurrentUserService currentUserService)
    {
        _unitOfWork = unitOfWork;
        _currentUserService = currentUserService;
    }

    public async Task<IEnumerable<CalendarEventDto>> Handle(GetDailyCalendarQuery request, CancellationToken cancellationToken)
    {
